...
public class ImageGrid {
    ...
    public BufferedImage build() {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (i % cols == 0) {
                startX = padding;
                startY = startY + runningHeight + padding;
            } else {
                startX = startX + gridImage.getWidth() + padding;
                rowHeight = Math.max(gridImage.getHeight(), rowHeight);
            }
    ...
    private int calculateNeededWidth(List<BufferedImage> elements, int cols) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            if (i > 0 && i % cols == 0) {
                neededRowWidth = Math.max(neededRowWidth, lastRowWidth);
                lastRowWidth = 0;
            }
            lastRowWidth += (elements.get(i).getWidth() + (padding));
        }
