...
class EmotionRecognitionSpecification extends Specification {

    def "Detect a face from an image URL and display the emotions"() {
        setup:
        EmotionBuilder emotionBuilder = new EmotionBuilder(CognitiveContext.build(System.getProperty("azure.cognitive.emotion.subscriptionKey")))
        when:
    ...
    def "Detect a face from an inputstream image and display the emotions"() {
        setup:
        EmotionBuilder emotionBuilder = new EmotionBuilder(CognitiveContext.build(System.getProperty("azure.cognitive.emotion.subscriptionKey")))
        when:
        List<Emotion> emotions = emotionBuilder.emotionRecognition(inputStream).withResult()
        then:
        emotions.size() == 1
        Emotion emotion = emotions[0]
        println emotion
